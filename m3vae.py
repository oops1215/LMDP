...
M3VAE（Multimodal Neuroimaging Representation Fusion）
	M3VAE：MRI/PET 双编码器 + PoE 融合 + 解码器（处理模态缺失）
论文 Section III.B：
...
import torch.utils.checkpoint  # 显式导入：use_checkpoint=True 时需要，避免依赖间接导入
...
class ImageEncoder3D(nn.Module):
    ...
    def set_cam_enabled(self, enabled: bool = True) -> None:
        """启用/关闭 Grad-CAM 缓存；默认关闭，不影响训练速度。"""
        self._cam_enabled = enabled
        if not enabled:
    ...
    def get_cam_features(self) -> Optional[torch.Tensor]:
        return self._cam_features

    def get_cam_gradients(self) -> Optional[torch.Tensor]:
        return self._cam_gradients

    def _save_cam_gradients(self, grad: torch.Tensor) -> None:
    ...
    def _run_encoder(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self._cam_enabled:
            self._cam_features = h
            if h.requires_grad:
                h.register_hook(self._save_cam_gradients)
        return h
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        h = self.fc(h)
        mu     = self.fc_mu(h)
        logvar = self.fc_logvar(h).clamp(-4.0, -1.5)  # σ²∈[0.018,0.223]，精度T≥4.48，保证有图时编码器一定有贡献
        return mu, logvar
...
class M3VAE(nn.Module):
    ...
    @staticmethod
    def compute_modality_contribution_tensors(
        mri_avail: torch.Tensor,
        pet_avail: torch.Tensor,
        mri_logvar: torch.Tensor,
        pet_logvar: torch.Tensor,
    ) -> Dict[str, torch.Tensor]:
        """返回逐样本/逐 latent 维度的 PoE 精度贡献，用于反事实解释。"""
        precision_mri = torch.exp(-mri_logvar.clamp(-10.0, 10.0)) * mri_avail.float().unsqueeze(1)
        precision_pet = torch.exp(-pet_logvar.clamp(-10.0, 10.0)) * pet_avail.float().unsqueeze(1)
        precision_prior = torch.ones_like(precision_mri)
        precision_total = (precision_prior + precision_mri + precision_pet).clamp(min=1e-8)

        contrib_mri = precision_mri / precision_total
        contrib_pet = precision_pet / precision_total
        contrib_prior = precision_prior / precision_total

        return {
            "precision_mri": precision_mri,
            "precision_pet": precision_pet,
            "precision_prior": precision_prior,
            "precision_total": precision_total,
            "contrib_mri": contrib_mri,
            "contrib_pet": contrib_pet,
            "contrib_prior": contrib_prior,
            "contrib_mri_sample": contrib_mri.mean(dim=1),
            "contrib_pet_sample": contrib_pet.mean(dim=1),
            "contrib_prior_sample": contrib_prior.mean(dim=1),
        }
    ...
    def vae_loss(self,
        ...
        # CONTRIB_FLOOR_WEIGHT 现由 config 控制（默认 0.0=关闭）：
        # 该项直接优化"精度权重≥40%"的 proxy，与真实目标 ΔIMG 脱钩。
        total = Config.RECON_WEIGHT * avg_recon_loss + Config.KL_WEIGHT * kl_reg + Config.CONTRIB_FLOOR_WEIGHT * contrib_loss
        return total, avg_recon_loss.detach().item(), kl_reg.detach().item()

    def encode_for_explain(self,
                           mri: Optional[torch.Tensor],
                           pet: Optional[torch.Tensor],
                           mri_avail: torch.Tensor,
                           pet_avail: torch.Tensor,
                           enable_cam: bool = False) -> Dict[str, torch.Tensor]:
        """
        解释/反事实专用编码接口：返回模态 latent、PoE 融合分布和逐样本贡献率。

        enable_cam=True 时会缓存 conv5 feature map/gradient，供 3D Grad-CAM 使用。
        """
        self.mri_encoder.set_cam_enabled(enable_cam)
        self.pet_encoder.set_cam_enabled(enable_cam)
        if enable_cam:
            self.mri_encoder.clear_cam_cache()
            self.pet_encoder.clear_cam_cache()
        ...
        if no_img.any():
            fused_mu = fused_mu.clone()
            fused_logvar = fused_logvar.clone()
            fused_mu[no_img] = 0.0
            fused_logvar[no_img] = 0.0

        contrib = self.compute_modality_contribution_tensors(
    ...
    def get_fused_mu(self,
        ...
        返回: (B, latent_dim)
        """
        explain = self.encode_for_explain(mri, pet, mri_avail, pet_avail, enable_cam=False)
        return explain["fused_mu"]
    ...
    def forward(self,
        ...
        if return_explain:
            explain = {
                ...
                "pet_logvar": pet_logvar,
                "modality_mask": mask,
                **self.compute_modality_contribution_tensors(
                    mri_avail, pet_avail, mri_logvar, pet_logvar),
            }
            return fused_mu, loss, contribs, avg_recon, avg_kl, explain
